...
namespace RedisEasyClient
{
    public static class Connector
    {
        ...
        {
            var db = Redis.GetDatabase();
            db.StringSet(key.ToLower(), JsonConvert.SerializeObject(value), expires);
        }
        public static void StoreSigleKeyOnCacheDecimal(string key, decimal value)
        {
            var db = Redis.GetDatabase();
            db.StringSet(key.ToLower(), JsonConvert.SerializeObject(value));
        }
        public static T GetSigleKeyFromCache<T>(string key)
        {
            var db = Redis.GetDatabase();
            var strObj = db.StringGet(key.ToLower());
            if (string.IsNullOrEmpty(strObj))
                return default(T);
            return JsonConvert.DeserializeObject<T>(strObj);
        }
        ...
        public static List<T> GetAllList<T>()
        {
            try
            {
                var tipo = GetObjName((typeof(T)).FullName, (typeof(T)).Name);
                var db = Redis.GetDatabase();
        ...
        public static string GetObjName(string fullname, string singlename)
        {
            if (fullname.Contains("System.Collections.Generic.List"))
            {
                ...
                var name = regex.Match(fullname);
                if (!string.IsNullOrEmpty(name.Value))
                {
                    var ar = name.Value.Split('.');
                    return ar.Last();
                }
            }
            return singlename;
